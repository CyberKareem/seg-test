...
.DESCRIPTION
  ...
  Usage:
    ...
    .\seg-test.ps1 -Cleanup                                # remove the kit, KEEP evidence (ADMIN)
    .\seg-test.ps1 -Cleanup -IncludeEvidence               # ...and delete evidence too (prompts)
  ...
  Cleanup (end of engagement - leave no tooling on the client host):
    -Cleanup            uninstall what this kit installed, delete tools\, clients\,
                        cde-all.txt and finally the script itself. evidence\ is KEPT.
                        Only uninstalls Nmap/Npcap if THIS kit installed them - a
                        pre-existing client install is left alone. Needs admin to
                        uninstall; the uninstallers are interactive, like the installers.
    -IncludeEvidence    with -Cleanup, ALSO delete evidence\ and the kit folder.
                        Prompts for the folder name first. Only after the evidence is
                        off the host - it cannot be regenerated without re-testing.
    -KeepDeps           with -Cleanup, remove the kit but leave nmap and Npcap installed.
    -ForceUninstall     with -Cleanup, uninstall Nmap/Npcap even if this kit did not
                        install them. Use only when you are certain.
  ...
  Cleanup writes a receipt into evidence\cleanup-<timestamp>.txt recording what was
  uninstalled and removed, so the audit trail shows when the tooling left the host.
...
# Records which products THIS kit installed. Cleanup uninstalls only what is listed here,
# so a client's pre-existing Nmap/Npcap is never removed.
# Deliberately at the kit ROOT, not inside tools\: cleanup deletes tools\ even when the
# uninstall could not run (not elevated), and losing the marker would mean a later cleanup
# no longer knows what this kit installed.
$InstallMarker = Join-Path $ScriptDir 'installed-by-seg-test.json'

# ------------------------------------------------------------------- helpers

function Write-Head($Text) { Write-Host $Text }

# ISO-8601 with offset, matching bash `date -Is`. InvariantCulture because ':' in a .NET
# format string is the culture's time separator and 'yyyy' follows the culture calendar.
# Defined here (not in the scan body) so -Init/-Check/-Cleanup can use it too.
...
function Get-NmapInstaller {
  New-KitDirectory -Path $ToolsDir
  $d   = $DEPS.Nmap
...
function Install-Nmap {
  ...
  Write-Head '============================================================'
  # Snapshot Npcap BEFORE the installer runs. Presence is judged by the registry
  # uninstall entry, not by Test-Npcap: Test-Npcap reports whether the driver is
  # RUNNING, which cannot distinguish "we just installed it" from "the client already
  # had it, stopped" - and plenty of hosts already ship Npcap with Wireshark.
  # The marker is the only thing protecting a client's own driver at cleanup time.
  $hadNpcap = @(Get-UninstallEntry -NameLike 'Npcap*').Count -gt 0
  ...
  $proc.WaitForExit()
  if (Get-NmapPath) {
    Write-Head (">>> nmap installed: {0}" -f (Get-NmapPath))
    Set-InstallMarker -Product 'Nmap'   # sound: the early return above proved nmap was absent
    if ($hadNpcap) {
      Write-Head '>>> Npcap was already on this host before the install - NOT claiming it.'
      Write-Head '    -Cleanup will leave the client Npcap in place.'
    } else {
      # Claim it only on a genuine absent -> present transition, and poll: driver
      # registration can lag the installer exiting by a few seconds.
      $gotNpcap = $false
      foreach ($try in 1..6) {
        if (@(Get-UninstallEntry -NameLike 'Npcap*').Count -gt 0) { $gotNpcap = $true; break }
        Start-Sleep -Seconds 2
      }
      if ($gotNpcap) { Set-InstallMarker -Product 'Npcap' }
    }
  }
...
function Install-Npcap {
  ...
  $hadNpcapBefore = @(Get-UninstallEntry -NameLike 'Npcap*').Count -gt 0
  New-KitDirectory -Path $ToolsDir
  $d   = $DEPS.Npcap
  $exe = Join-Path $ToolsDir $d.File
  if (Test-Path -LiteralPath $exe) {
  ...
  if ($ok) {
    Write-Head '>>> Npcap installed and driver is running.'
    # Same rule as Install-Nmap: only claim it if it was genuinely absent beforehand.
    # (The early return above only proves the driver was not RUNNING, not that Npcap
    # was not installed.)
    if ($hadNpcapBefore) {
      Write-Head '>>> Npcap was already installed here - NOT claiming it for cleanup.'
    } else {
      Set-InstallMarker -Product 'Npcap'
    }
  }
  else { Write-Head '!!! Npcap driver still not running. A reboot is usually required - reboot, then re-run -Check.' }
}

# ------------------------------------------------------- install marker / cleanup

# Create a directory and mark it as ours. A directory that ALREADY EXISTS is left
# unclaimed, so cleanup will never recurse-delete a folder the client already had
# (e.g. the operator unzipped the kit into an existing C:\tools).
function New-KitDirectory {
  param([string]$Path)
  if (Test-Path -LiteralPath $Path) { return }
  New-Item -ItemType Directory -Path $Path -Force | Out-Null
  try { Write-Evidence -Path (Join-Path $Path '.seg-test-owned') -Text (Get-Stamp) -Quiet } catch { }
}
function Test-KitOwned {
  param([string]$Path)
  return (Test-Path -LiteralPath (Join-Path $Path '.seg-test-owned'))
}
...
function Get-InstallMarker {
  ...
  if (Test-Path -LiteralPath $InstallMarker) {
    try {
      $j = (Get-Content -LiteralPath $InstallMarker -Raw -Encoding UTF8) | ConvertFrom-Json
      foreach ($pr in $j.PSObject.Properties) { $data[$pr.Name] = [string]$pr.Value }
    } catch { }
  }
  return $data
}
...
function Set-InstallMarker {
  ...
  $data = Get-InstallMarker
  $data[$Product] = (Get-Stamp)
  Write-Evidence -Path $InstallMarker -Text ([pscustomobject]$data | ConvertTo-Json) -Quiet
}

# Find uninstall entries by DisplayName pattern. StrictMode-safe property access.
function Get-UninstallEntry {
  ...
  foreach ($r in $roots) {
    ...
    foreach ($k in @(Get-ChildItem -LiteralPath $r -ErrorAction SilentlyContinue)) {
      $pr = $null
      try { $pr = Get-ItemProperty -LiteralPath $k.PSPath -ErrorAction Stop } catch { continue }
      if (-not $pr.PSObject.Properties['DisplayName']) { continue }
      $dn = [string]$pr.DisplayName
      if ($dn -notlike $NameLike) { continue }
      $us = ''
      if ($pr.PSObject.Properties['UninstallString']) { $us = [string]$pr.UninstallString }
      $out.Add([pscustomobject]@{ DisplayName = $dn; UninstallString = $us })
    }
  }
  return $out.ToArray()
}

# Run an uninstaller and wait. Interactive, like the installers - the free Nmap/Npcap
# uninstallers have no silent mode either. Never killed on a timer: aborting a
# kernel-driver uninstall mid-write can leave a half-registered NDIS filter.
function Invoke-Uninstaller {
  param([string]$UninstallString, [string]$Label)
  if ([string]::IsNullOrWhiteSpace($UninstallString)) {
    Write-Head ("  !! {0}: no UninstallString in the registry; remove it manually." -f $Label)
    return $false
  }
  $exe = ''; $rest = ''
  if     ($UninstallString -match '^\s*"([^"]+)"\s*(.*)$')   { $exe = $Matches[1]; $rest = $Matches[2].Trim() }
  elseif ($UninstallString -match '^\s*(\S+\.exe)\s*(.*)$')  { $exe = $Matches[1]; $rest = $Matches[2].Trim() }
  else                                                       { $exe = $UninstallString.Trim() }
  if (-not (Test-Path -LiteralPath $exe)) {
    Write-Head ("  !! {0}: uninstaller not found at {1}; remove it manually." -f $Label, $exe)
    return $false
  }
  Write-Head ("  >> Launching {0} uninstaller (interactive - click through it)." -f $Label)
  try {
    if ($rest) { $proc = Start-Process -FilePath $exe -ArgumentList $rest -PassThru }
    else       { $proc = Start-Process -FilePath $exe -PassThru }
    $proc.WaitForExit()
    return $true
  } catch {
    Write-Head ("  !! {0}: uninstaller failed to launch: {1}" -f $Label, $_.Exception.Message)
    return $false
  }
}
...
function Invoke-Cleanup {
  ...
  # The scan writes to $Root\evidence, where ROOT may be overridden per client in
  # clients\<name>.env. Cleanup must not assume $ScriptDir\evidence: with an override
  # it would "preserve" an empty local folder while the real evidence sits elsewhere,
  # and -IncludeEvidence would then wipe the kit folder without ever prompting.
  $localEvidence = Join-Path $ScriptDir 'evidence'
  $externalRoots = New-Object System.Collections.Generic.List[string]
  if (Test-Path -LiteralPath $ClientsDir) {
    foreach ($ef in @(Get-ChildItem -LiteralPath $ClientsDir -Filter '*.env' -ErrorAction SilentlyContinue)) {
      $cfg = $null
      # Import-ClientEnv throws on a malformed array, and EAP=Stop would abort cleanup
      # before it did anything. Each file gets its own guard.
      try { $cfg = Import-ClientEnv -Path $ef.FullName } catch {
        Write-Head ("  !! could not parse {0}: {1}" -f $ef.Name, $_.Exception.Message)
        continue
      }
      $r = ''
      if ($cfg.ContainsKey('ROOT')) { $r = [string]$cfg['ROOT'] }
      if ([string]::IsNullOrWhiteSpace($r)) { continue }
      $cand = Join-Path $r 'evidence'
      if ($cand -eq $localEvidence) { continue }
      if (-not $externalRoots.Contains($cand)) { $externalRoots.Add($cand) }
    }
  }

  function Count-Items($dir) {
    if (-not (Test-Path -LiteralPath $dir)) { return 0 }
    return @(Get-ChildItem -LiteralPath $dir -Force -ErrorAction SilentlyContinue).Count
  }
  $localCount = Count-Items $localEvidence
  $extTotal = 0
  foreach ($e in $externalRoots) { $extTotal += (Count-Items $e) }

  Write-Head '============================================================'
  Write-Head ' CLEANUP - remove the seg-test kit from this host'
  Write-Head '============================================================'
  Write-Head (" Kit folder : {0}" -f $ScriptDir)
  Write-Head (" Evidence   : {0} item(s) in {1}" -f $localCount, $localEvidence)
  foreach ($e in $externalRoots) {
    Write-Head ("            : {0} item(s) in {1}   <- ROOT override" -f (Count-Items $e), $e)
  }
  Write-Head ''

  if ($externalRoots.Count -gt 0) {
    Write-Head '>>> A client config sets ROOT, so some evidence lives OUTSIDE this kit folder.'
    Write-Head '>>> Cleanup will NOT touch anything outside the kit folder. Those paths are'
    Write-Head '>>> listed above - remove them yourself once the evidence is off the host.'
    Write-Head ''
  }

  # --- Evidence is the deliverable. Confirm unconditionally. ----------------
  if ($IncludeEvidence) {
    # Deliberately NOT gated on the item count: -IncludeEvidence also schedules
    # 'rd /s /q' on the whole kit folder, which would take anything else the operator
    # left in it (ROE, notes, manual captures) even when evidence\ is empty.
    Write-Head '!!! -IncludeEvidence will PERMANENTLY and IRREVERSIBLY delete:'
    Write-Head ("!!!   the ENTIRE kit folder: {0}   (rd /s /q - no recycle bin)" -f $ScriptDir)
    Write-Head ("!!!   evidence\: {0} item(s) in {1}" -f $localCount, $localEvidence)
    Write-Head '!!! Anything else in that folder (ROE, notes, manual captures) goes with it.'
    Write-Head '!!! This is one-shot PCI DSS 11.4.5 evidence. It cannot be regenerated'
    Write-Head '!!! without re-running the test from inside every source VLAN.'
    if ($externalRoots.Count -gt 0) {
      Write-Head '!!! NOTE: the ROOT-override paths above will NOT be deleted - handle them yourself.'
    }
    Write-Head ''
    $want = Split-Path -Leaf $ScriptDir
    $typed = Read-Host ("To confirm, type the kit folder name exactly ({0})" -f $want)
    if ($typed -ne $want) {
      Write-Head '>>> Confirmation did not match. Nothing was deleted. Aborting.'
      return 1
    }
  } elseif ($localCount -gt 0 -or $extTotal -gt 0) {
    Write-Head (">>> evidence\ will be PRESERVED ({0} local item(s))." -f $localCount)
    Write-Head '>>> Copy it off this host before you disconnect - it is the engagement deliverable.'
    Write-Head '>>> (Pass -IncludeEvidence only once it is safely off the host.)'
    Write-Head ''
  }

  # --- Uninstall only what THIS kit installed -------------------------------
  $marker = Get-InstallMarker
  $uninstalled = New-Object System.Collections.Generic.List[string]
  $skipped     = New-Object System.Collections.Generic.List[string]

  if ($KeepDeps) {
    Write-Head '>>> -KeepDeps: leaving nmap and Npcap installed.'
    $skipped.Add('Nmap and Npcap (-KeepDeps)')
  } else {
    foreach ($prod in @('Nmap','Npcap')) {
      if (-not $marker.ContainsKey($prod) -and -not $ForceUninstall) {
        Write-Head ("  -- {0}: not installed by this kit; leaving it alone (use -ForceUninstall to override)." -f $prod)
        $skipped.Add(("{0} (pre-existing - not installed by this kit)" -f $prod))
        continue
      }
      $entries = @(Get-UninstallEntry -NameLike ("{0}*" -f $prod))
      if ($entries.Count -eq 0) {
        $skipped.Add(("{0} (no uninstall entry found)" -f $prod))
        continue
      }
      if (-not (Test-IsAdmin)) {
        Write-Head ("  !! {0} uninstall needs Administrator - skipping." -f $prod)
        $skipped.Add(("{0} (NOT REMOVED - not elevated)" -f $prod))
        continue
      }
      foreach ($en in $entries) {
        if (Invoke-Uninstaller -UninstallString $en.UninstallString -Label $en.DisplayName) {
          $uninstalled.Add($en.DisplayName)
        } else {
          $skipped.Add(("{0} (uninstaller failed)" -f $en.DisplayName))
        }
      }
    }
  }
  ...
  # --- Remove kit files -----------------------------------------------------
  # A directory is recurse-deleted only if THIS kit created it (.seg-test-owned).
  # If the operator dropped the script into a folder that already had tools\ or
  # clients\, only the files the kit itself wrote are removed.
  $removed = New-Object System.Collections.Generic.List[string]
  $kept    = New-Object System.Collections.Generic.List[string]

  function Remove-KitPath($path) {
    if (-not (Test-Path -LiteralPath $path)) { return }
    try {
      Remove-Item -LiteralPath $path -Recurse -Force -ErrorAction Stop
      $removed.Add($path)
    } catch {
      Write-Head ("  !! could not remove {0}: {1}" -f $path, $_.Exception.Message)
      $kept.Add(("{0} (removal failed)" -f $path))
    }
  }

  foreach ($d in @($ToolsDir, $ClientsDir)) {
    if (-not (Test-Path -LiteralPath $d)) { continue }
    if (Test-KitOwned $d) {
      Remove-KitPath $d
    } else {
      Write-Head ("  -- {0} existed before this kit; removing only our own files from it." -f $d)
      $ours = New-Object System.Collections.Generic.List[string]
      if ($d -eq $ToolsDir) {
        $ours.Add((Join-Path $d $DEPS.Nmap.File))
        $ours.Add((Join-Path $d $DEPS.Npcap.File))
        $ours.Add((Join-Path $d 'nmap'))
      } else {
        foreach ($f in @(Get-ChildItem -LiteralPath $d -Filter '*.env' -ErrorAction SilentlyContinue)) {
          $ours.Add($f.FullName)
        }
      }
      foreach ($o in $ours) { Remove-KitPath $o }
      $kept.Add(("{0} (pre-existing folder - left in place)" -f $d))
    }
  }
  Remove-KitPath (Join-Path $ScriptDir 'cde-all.txt')
  Remove-KitPath $InstallMarker
  if ($IncludeEvidence) { Remove-KitPath $localEvidence }

  # --- Cleanup receipt ------------------------------------------------------
  $evState = if ($IncludeEvidence) { 'DELETED (-IncludeEvidence)' } else { 'preserved at ' + $localEvidence }
  $receiptLines = New-Object System.Collections.Generic.List[string]
  $receiptLines.Add('# seg-test cleanup receipt')
  $receiptLines.Add('TIMESTAMP: '   + (Get-Stamp))
  $receiptLines.Add('HOSTNAME: '    + $env:COMPUTERNAME)
  $receiptLines.Add('USER: '        + $env:USERNAME)
  $receiptLines.Add('KIT: '         + $ScriptDir)
  $receiptLines.Add('UNINSTALLED: ' + $(if ($uninstalled.Count) { ($uninstalled -join '; ') } else { '(none)' }))
  $receiptLines.Add('SKIPPED: '     + $(if ($skipped.Count)     { ($skipped -join '; ') }     else { '(none)' }))
  $receiptLines.Add('REMOVED: '     + $(if ($removed.Count)     { ($removed -join '; ') }     else { '(none)' }))
  $receiptLines.Add('KEPT: '        + $(if ($kept.Count)        { ($kept -join '; ') }        else { '(none)' }))
  $receiptLines.Add('EVIDENCE: '    + $evState)
  foreach ($e in $externalRoots) { $receiptLines.Add('EVIDENCE_EXTERNAL (not touched): ' + $e) }
  $receiptLines.Add('SCRIPT: deleted at end of cleanup')

  if (-not $IncludeEvidence -and (Test-Path -LiteralPath $localEvidence)) {
    try {
      $rn = 'cleanup-' + [datetime]::Now.ToString('yyyy-MM-dd-HHmm',[cultureinfo]::InvariantCulture) + '.txt'
      Write-EvidenceLines -Path (Join-Path $localEvidence $rn) -Lines $receiptLines.ToArray() -Quiet
    } catch { }
  }

  Write-Head ''
  Write-Head '--- Cleanup summary ---'
  $receiptLines | ForEach-Object { Write-Head ('  ' + $_) }
  Write-Head ''

  # --- Finally, the script itself -------------------------------------------
  $self = $PSCommandPath
  if ([string]::IsNullOrWhiteSpace($self)) { $self = $MyInvocation.MyCommand.Path }
  Write-Head (">>> Removing the script: {0}" -f $self)
  try {
    Remove-Item -LiteralPath $self -Force -ErrorAction Stop
  } catch {
    # Windows can hold the .ps1 open. Hand the delete to a detached cmd that waits
    # until this process has exited.
    try {
      Start-Process -FilePath 'cmd.exe' `
        -ArgumentList ('/c timeout /t 3 /nobreak >nul & del /f /q "{0}"' -f $self) `
        -WindowStyle Hidden | Out-Null
      Write-Head '>>> Script is locked by this process; scheduled its deletion in ~3s.'
    } catch {
      Write-Head ("!!! Could not remove the script: {0}" -f $_.Exception.Message)
      Write-Head ("!!! Delete it manually: {0}" -f $self)
    }
  }
  ...
  if ($IncludeEvidence) {
    try {
      Start-Process -FilePath 'cmd.exe' `
        -ArgumentList ('/c timeout /t 6 /nobreak >nul & rd /s /q "{0}"' -f $ScriptDir) `
        -WindowStyle Hidden -WorkingDirectory (Split-Path -Parent $ScriptDir) | Out-Null
      Write-Head (">>> Scheduled removal of the kit folder: {0}" -f $ScriptDir)
    } catch { }
  }
  ...
  Write-Head '============================================================'
  if ($IncludeEvidence) {
    Write-Head ' CLEANUP DONE - kit and evidence removed.'
  } else {
    Write-Head ' CLEANUP DONE - kit removed.'
    Write-Head (" Evidence PRESERVED at: {0}" -f $localEvidence)
    Write-Head ' Copy it off this host now if you have not already.'
  }
  foreach ($e in $externalRoots) { Write-Head (" NOT TOUCHED (ROOT override): {0}" -f $e) }
  if ($skipped.Count) { Write-Head ' Check the SKIPPED line above - some items were left behind.' }
  Write-Head '============================================================'
  return 0
}
...
function Invoke-Init {
  ...
  New-KitDirectory -Path $ClientsDir
...
'@
  ...
  New-KitDirectory -Path (Join-Path $ScriptDir 'evidence')
...
if ($Cleanup) { exit (Invoke-Cleanup) }
